...
sub main
{
    ...
    my $tree = $parser->parse($remotepage);
    my (@links,@as);
    Flutterby::Tree::Find::FindNodeArrayRecurse('link', \@links);
    Flutterby::Tree::Find::FindNodeArrayRecurse('a', \@as);
    @as = grep { $_->[1]->

    if ($remotepage =~ /\<link\s*href="(.*)" rel="webmention" />
<a href="http://alice.host/webmention-endpoint" rel="webmention" />


11    $dbh = DBI->connect($configuration->{-database},
